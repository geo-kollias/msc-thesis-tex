Section: ft-declosurify big picture
...
Section: ft-declosurify implementation

Subsection: macroMap/macroForeach definition

macroForeach method, as the standard foreach, is meant to traverse all elements
of the collection, and apply the given operation, f, to each element. The
invocation of f is done for its side effect only; in fact any function result of
f is discarded. Similarly, macroMap, as the standard map, traverses all elements
of the collection, and apply the given operation, f, to each element but, also,
it always returns a collection whose type depends on the f. We could see
macroMap's functionality as a superset of the macroForeach's functionality and
that's true in the implementation side too. The same code handles both
cases but, in the case of macroForeach, we ignore the result. For that reason
and without any loss of generality we will be referring only to the macroMap in
the rest of the text.

macroMap is defined in scala.collection.TraversableLike trait, where the
standard library's map method is defined too. It's definition is:

``def macroMap[B, That](f0: A => B)(implicit bf: CanBuildFrom[Repr, B, That]):
That = ???''

Its definition is exactly the same as the map's one. Regarding the type
...
Subsection: scala.collection.IndexedSeq Transformation
...
[TODO: move builder description to ``ft-declosurify big picture`` section]
...
In the next chapter will see how much faster this version is compared to List(1,
2, 3).map(_ + 1).
...
categories. Roughly it includes all kinds of Sets, Maps, Buffers and more
specialized sequences. Obviously, since it includes such a broad range of
...
example, any macroMap applications on scala.collection.immutable.Map like:

Map(1->"a", 2->"b", 3->"c").macroMap(_._2)

will be replaced from:

{
  private def local1(x$1: (Int, String)): String = x$1._2;
  private def builder1:
scala.collection.mutable.Builder[String,scala.collection.immutable.Iterable[
String]] = {
    val b:
scala.collection.mutable.Builder[String,scala.collection.immutable.Iterable[
String]] = immutable.this.Iterable.canBuildFrom[String].apply();
    b.sizeHint(scala.this.Predef.Map.apply[Int,
String](scala.this.Predef.any2ArrowAssoc[Int](1).->[String]("a"),
scala.this.Predef.any2ArrowAssoc[Int](2).->[String]("b"),
scala.this.Predef.any2ArrowAssoc[Int](3).->[String]("c")));
    b
  };
  val buf = builder1();
  val it = scala.this.Predef.Map.apply[Int,
String](scala.this.Predef.any2ArrowAssoc[Int](1).->[String]("a"),
scala.this.Predef.any2ArrowAssoc[Int](2).->[String]("b"),
scala.this.Predef.any2ArrowAssoc[Int](3).->[String]("c")).toIterator;
  while(it.hasNext){
    buf.+=(local1(it.next));
  };
  buf.result
